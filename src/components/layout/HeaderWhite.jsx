  ...
  return (
    ...
    >
      <div className="lg:px-[6.25rem] md:px-8 px-5 text-meet_white flex h-full items-center">
        ...
        {/* 네비게이션 메뉴 */}
        <div
          className={`lg:flex lg:justify-center lg:mr-12 absolute lg:static top-[3.5rem] left-0 w-full bg-meet_light_gray lg:bg-transparent transition-all duration-500 ease-in-out overflow-hidden ${
            isOpen
              ? "max-h-[300px] pb-16 opacity-100"
              : "max-h-0 pb-0 opacity-0"
          } lg:max-h-none lg:opacity-100 lg:pb-0`}
        >
          <ul className="lg:flex lg:space-x-6 py-6 lg:text-meet_white flex flex-col lg:flex-row items-center space-y-4 lg:space-y-0 p-0 m-0 text-meet_menu">
            <li>고객지원</li>
            <li>포인트 환전</li>
            <li>앱 다운로드</li>
          </ul>
        ...
        >
          <button className="lg:w-32 px-5 py-[0.375rem] bg-meet_pink rounded-3xl font-bold flex items-center justify-center whitespace-nowrap">
            로그인
          </button>
        </div>
      </div>
